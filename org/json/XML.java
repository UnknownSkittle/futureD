...
public class XML {
    ...
    public static String escape(String string) {
        ...
        block7: for (int cp2 : XML.codePointIterator(string)) {
            ...
            if (XML.mustEscape(cp2)) {
                sb.append("&#x");
                sb.append(Integer.toHexString(cp2));
                sb.append(';');
                continue;
            }
    ...
    private static boolean parse(XMLTokener x2, JSONObject context, String name, XMLParserConfiguration config) throws JSONException {
        String string;
        ...
        if (token == GT) {
            while (true) {
                ...
                if (token == LT && XML.parse(x2, jsonobject, tagName, config)) break;
            }
