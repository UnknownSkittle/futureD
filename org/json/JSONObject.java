...
public class JSONObject {
    ...
    public BigInteger getBigInteger(String key) throws JSONException {
        ...
        throw new JSONException("JSONObject[" + JSONObject.quote(key) + "] could not be converted to BigInteger (" + object + ").");
    }
    ...
    public BigDecimal getBigDecimal(String key) throws JSONException {
        ...
        throw new JSONException("JSONObject[" + JSONObject.quote(key) + "] could not be converted to BigDecimal (" + object + ").");
    }
    ...
    public static String[] getNames(JSONObject jo2) {
        ...
        return jo2.keySet().toArray(new String[jo2.length()]);
    }
    ...
    static BigDecimal objectToBigDecimal(Object val, BigDecimal defaultValue) {
        ...
        if (val instanceof Double || val instanceof Float) {
            double d2 = ((Number)val).doubleValue();
            if (Double.isNaN(d2)) {
                return defaultValue;
            }
    ...
    private void populateMap(Object bean) {
        ...
        for (Method method : methods = includeSuperClass ? klass.getMethods() : klass.getDeclaredMethods()) {
            ...
            if (!Modifier.isPublic(modifiers) || Modifier.isStatic(modifiers) || method.getParameterTypes().length != 0 || method.isBridge() || method.getReturnType() == Void.TYPE || !this.isValidMethodName(method.getName()) || (key = this.getKeyNameFromMethod(method)) == null || key.isEmpty()) continue;
            try {
    ...
    private static <A extends Annotation> A getAnnotation(Method m2, Class<A> annotationClass) {
        ...
        for (Class<?> i2 : c2.getInterfaces()) {
            try {
                Method im2 = i2.getMethod(m2.getName(), m2.getParameterTypes());
                return JSONObject.getAnnotation(im2, annotationClass);
    ...
    private static int getAnnotationDepth(Method m2, Class<? extends Annotation> annotationClass) {
        ...
        for (Class<?> i2 : c2.getInterfaces()) {
            try {
                Method im2 = i2.getMethod(m2.getName(), m2.getParameterTypes());
                int d2 = JSONObject.getAnnotationDepth(im2, annotationClass);
                if (d2 <= 0) continue;
                return d2 + 1;
            } catch (SecurityException ex2) {
            } catch (NoSuchMethodException ex3) {
