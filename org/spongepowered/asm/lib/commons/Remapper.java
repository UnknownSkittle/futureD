...
public abstract class Remapper {
    ...
    private Type mapType(Type t2) {
        switch (t2.getSort()) {
            ...
            case 10: {
                String s3 = this.map(t2.getInternalName());
                return s3 != null ? Type.getObjectType(s3) : t2;
            }
            case 11: {
                return Type.getMethodType(this.mapMethodDesc(t2.getDescriptor()));
            }
        }
        return t2;
    }
    ...
    public String mapSignature(String signature, boolean typeSignature) {
        ...
        SignatureVisitor a2 = this.createSignatureRemapper(w2);
        if (typeSignature) {
            r2.acceptType(a2);
        } else {
            r2.accept(a2);
