...
extends FieldVisitor {
    ...
    public FieldNode(int api2, int access, String name, String desc, String signature, Object value) {
        super(api2);
        this.access = access;
        this.name = name;
        this.desc = desc;
    ...
    public void accept(ClassVisitor cv2) {
        AnnotationNode an2;
        int i2;
        FieldVisitor fv2 = cv2.visitField(this.access, this.name, this.desc, this.signature, this.value);
        if (fv2 == null) {
            return;
        }
        ...
        for (i2 = 0; i2 < n2; ++i2) {
            ...
            an2.accept(fv2.visitAnnotation(an2.desc, true));
        }
        n2 = this.invisibleAnnotations == null ? 0 : this.invisibleAnnotations.size();
        for (i2 = 0; i2 < n2; ++i2) {
            an2 = this.invisibleAnnotations.get(i2);
            an2.accept(fv2.visitAnnotation(an2.desc, false));
        }
        n2 = this.visibleTypeAnnotations == null ? 0 : this.visibleTypeAnnotations.size();
        for (i2 = 0; i2 < n2; ++i2) {
            an2 = this.visibleTypeAnnotations.get(i2);
            an2.accept(fv2.visitTypeAnnotation(((TypeAnnotationNode)an2).typeRef, ((TypeAnnotationNode)an2).typePath, ((TypeAnnotationNode)an2).desc, true));
        }
        n2 = this.invisibleTypeAnnotations == null ? 0 : this.invisibleTypeAnnotations.size();
        for (i2 = 0; i2 < n2; ++i2) {
            an2 = this.invisibleTypeAnnotations.get(i2);
            an2.accept(fv2.visitTypeAnnotation(((TypeAnnotationNode)an2).typeRef, ((TypeAnnotationNode)an2).typePath, ((TypeAnnotationNode)an2).desc, false));
        }
        n2 = this.attrs == null ? 0 : this.attrs.size();
        for (i2 = 0; i2 < n2; ++i2) {
            fv2.visitAttribute(this.attrs.get(i2));
        }
        fv2.visitEnd();
    }
}
