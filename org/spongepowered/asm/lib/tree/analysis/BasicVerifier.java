    ...
    public BasicValue naryOperation(AbstractInsnNode insn, List<? extends BasicValue> values) throws AnalyzerException {
        ...
        if (opcode == 197) {
            for (int i2 = 0; i2 < values.size(); ++i2) {
                if (BasicValue.INT_VALUE.equals(values.get(i2))) continue;
                throw new AnalyzerException(insn, null, BasicValue.INT_VALUE, values.get(i2));
            }
        } else {
            int i3 = 0;
            int j2 = 0;
            if (opcode != 184 && opcode != 186) {
                ...
                if (!this.isSubTypeOf(values.get(i3++), this.newValue(owner))) {
            ...
            while (i3 < values.size()) {
                ...
                BasicValue expected = this.newValue(args[j2++]);
                if (this.isSubTypeOf(encountered = values.get(i3++), expected)) continue;
                throw new AnalyzerException(insn, "Argument " + j2, expected, encountered);
            }
        }
