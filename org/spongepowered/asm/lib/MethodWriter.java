...
extends MethodVisitor {
    ...
    MethodWriter(ClassWriter cw2, int access, String name, String desc, String signature, String[] exceptions, int compute) {
        super(327680);
        if (cw2.firstMethod == null) {
            cw2.firstMethod = this;
        } else {
            cw2.lastMethod.mv = this;
        }
        cw2.lastMethod = this;
        this.cw = cw2;
        this.access = access;
    ...
    public AnnotationVisitor visitTypeAnnotation(int typeRef, TypePath typePath, String desc, boolean visible) {
        ByteVector bv2 = new ByteVector();
        AnnotationWriter.putTarget(typeRef, typePath, bv2);
        bv2.putShort(this.cw.newUTF8(desc)).putShort(0);
        AnnotationWriter aw2 = new AnnotationWriter(this.cw, true, bv2, bv2, bv2.length - 2);
        if (visible) {
            aw2.next = this.tanns;
            this.tanns = aw2;
        } else {
            aw2.next = this.itanns;
            this.itanns = aw2;
        }
        return aw2;
    }
    ...
    public AnnotationVisitor visitInsnAnnotation(int typeRef, TypePath typePath, String desc, boolean visible) {
        ...
        AnnotationWriter aw2 = new AnnotationWriter(this.cw, true, bv2, bv2, bv2.length - 2);
        if (visible) {
            aw2.next = this.ctanns;
            this.ctanns = aw2;
        } else {
            aw2.next = this.ictanns;
            this.ictanns = aw2;
        }
        return aw2;
    }
    ...
    public void visitLocalVariable(String name, String desc, String signature, Label start, Label end, int index) {
        ...
        if (this.compute != 3 && (n2 = index + ((c2 = desc.charAt(0)) == 'J' || c2 == 'D' ? 2 : 1)) > this.maxLocals) {
            this.maxLocals = n2;
        }
    }
    ...
    public AnnotationVisitor visitLocalVariableAnnotation(int typeRef, TypePath typePath, Label[] start, Label[] end, int[] index, String desc, boolean visible) {
        ByteVector bv2 = new ByteVector();
        bv2.putByte(typeRef >>> 24).putShort(start.length);
        for (int i2 = 0; i2 < start.length; ++i2) {
            bv2.putShort(start[i2].position).putShort(end[i2].position - start[i2].position).putShort(index[i2]);
        }
        if (typePath == null) {
            bv2.putByte(0);
        } else {
            int length = typePath.b[typePath.offset] * 2 + 1;
            bv2.putByteArray(typePath.b, typePath.offset, length);
        }
        bv2.putShort(this.cw.newUTF8(desc)).putShort(0);
        AnnotationWriter aw2 = new AnnotationWriter(this.cw, true, bv2, bv2, bv2.length - 2);
        if (visible) {
    ...
    public void visitMaxs(int maxStack, int maxLocals) {
        if (this.compute == 0) {
            ...
            f2.initInputFrame(this.cw, this.access, Type.getArgumentTypes(this.descriptor), this.maxLocals);
            this.visitFrame(f2);
            int max = 0;
            ...
            while (handler != null) {
                ...
                while (l4 != e4) {
                    ...
                    b3.successor = h3;
                    if ((l4.status & 0x80) == 0) {
                        b3.next = l4.successors;
                        l4.successors = b3;
                    } else {
                        b3.next = l4.successors.next.next;
                        l4.successors.next.next = b3;
                    }
                    l4 = l4.successor;
                }
                handler = handler.next;
                ...
                while (l5 != null) {
                    if ((l5.status & 0x80) != 0) {
                        ...
                        if ((subroutine.status & 0x400) == 0) {
                            subroutine.visitSubroutine(null, (long)(++id2) / 32L << 32 | 1L << id2 % 32, this.subroutines);
                        }
                    }
                    l5 = l5.successor;
                }
            ...
            while (stack != null) {
                ...
                while (b4 != null) {
                    l6 = b4.successor;
                    if ((l6.status & 8) == 0) {
                        l6.inputStackTop = b4.info == Integer.MAX_VALUE ? 1 : start + b4.info;
                        l6.status |= 8;
                        l6.next = stack;
                        stack = l6;
                    }
    ...
    private void visitFrame(Frame f2) {
        ...
        int frameIndex = this.startFrame(f2.owner.position, nLocal, nStack);
        i2 = 0;
        while (nLocal > 0) {
            t2 = locals[i2];
            this.frame[frameIndex++] = t2;
        ...
        block8: while (true) {
            ...
            switch (this.descriptor.charAt(i2++)) {
                ...
                case '[': {
                    ...
                    this.frame[frameIndex++] = 0x1700000 | this.cw.addType(this.descriptor.substring(j2, ++i2));
                    continue block8;
                }
                case 'L': {
                    while (this.descriptor.charAt(i2) != ';') {
                    ...
                    this.frame[frameIndex++] = 0x1700000 | this.cw.addType(this.descriptor.substring(j2 + 1, i2++));
                    continue block8;
                }
            }
            break;
    ...
    private void writeFrameTypes(int start, int end) {
        for (int i2 = start; i2 < end; ++i2) {
            ...
            if ((t2 & 0xFF00000) == 0x1700000) {
                sb.append('L');
                sb.append(this.cw.typeTable[t2 & 0xFFFFF].strVal1);
                sb.append(';');
            } else {
                switch (t2 & 0xF) {
                    case 1: {
