...
public class Type {
    ...
    public static Type getType(Constructor<?> c2) {
        return Type.getType(Type.getConstructorDescriptor(c2));
    }

    public static Type getType(Method m2) {
        return Type.getType(Type.getMethodDescriptor(m2));
    }
    ...
    private static void getDescriptor(StringBuilder buf, Class<?> c2) {
        Class<?> d2 = c2;
        while (true) {
            if (d2.isPrimitive()) {
