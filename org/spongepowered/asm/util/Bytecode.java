...
public final class Bytecode {
    ...
    private static Pattern getMergeableAnnotationPattern() {
        ...
        for (int i2 = 0; i2 < MERGEABLE_MIXIN_ANNOTATIONS.length; ++i2) {
            ...
            sb.append(MERGEABLE_MIXIN_ANNOTATIONS[i2].getName().replace('.', '/'));
        }
        return Pattern.compile(sb.append(");$").toString());
    }

    public static void compareBridgeMethods(MethodNode a2, MethodNode b2) {
        ListIterator<AbstractInsnNode> ia = a2.instructions.iterator();
        ListIterator<AbstractInsnNode> ib = b2.instructions.iterator();
        int index = 0;
        while (ia.hasNext() && ib.hasNext()) {
            AbstractInsnNode na = ia.next();
            AbstractInsnNode nb2 = ib.next();
            if (!(na instanceof LabelNode)) {
                if (na instanceof MethodInsnNode) {
                    MethodInsnNode ma2 = (MethodInsnNode)na;
                    MethodInsnNode mb2 = (MethodInsnNode)nb2;
                    if (!ma2.name.equals(mb2.name)) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INVOKE_NAME, a2.name, a2.desc, index, na, nb2);
                    }
                    if (!ma2.desc.equals(mb2.desc)) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INVOKE_DESC, a2.name, a2.desc, index, na, nb2);
                    }
                } else {
                    if (na.getOpcode() != nb2.getOpcode()) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INSN, a2.name, a2.desc, index, na, nb2);
                    }
                    if (na instanceof VarInsnNode) {
                        VarInsnNode va2 = (VarInsnNode)na;
                        VarInsnNode vb2 = (VarInsnNode)nb2;
                        if (va2.var != vb2.var) {
                            throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_LOAD, a2.name, a2.desc, index, na, nb2);
                        }
                        ...
                        if (ta.getOpcode() == 192 && !ta.desc.equals(tb.desc)) {
                            throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_CAST, a2.name, a2.desc, index, na, nb2);
                        }
                    }
                }
            }
