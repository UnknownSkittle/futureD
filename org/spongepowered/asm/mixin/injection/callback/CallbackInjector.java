    ...
    protected void addTargetNode(Target target, List<InjectionNodes.InjectionNode> myNodes, AbstractInsnNode node, Set<InjectionPoint> nominators) {
        ...
        for (InjectionPoint ip : nominators) {
            ...
            if (existingId != null && !existingId.equals(id2)) {
                Injector.logger.warn("Conflicting id for {} insn in {}, found id {} on {}, previously defined as {}", new Object[]{Bytecode.getOpcodeName(node), target.toString(), id2, this.info, existingId});
                break;
            }
