...
implements IClassGenerator {
    ...
    public String registerInnerClass(MixinInfo owner, String originalName, MixinTargetContext context) {
        String id2 = String.format("%s%s", originalName, context);
        String ref = this.innerClassNames.get(id2);
        if (ref == null) {
            ref = InnerClassGenerator.getUniqueReference(originalName, context);
            this.innerClassNames.put(id2, ref);
    ...
    private byte[] generate(InnerClassInfo info) {
        try {
            ...
            ClassReader cr2 = new ClassReader(info.getClassBytes());
            MixinClassWriter cw2 = new MixinClassWriter(cr2, 0);
            cr2.accept(new InnerClassAdapter((ClassVisitor)cw2, info), 8);
            return cw2.toByteArray();
        } catch (InvalidMixinException ex2) {
            throw ex2;
        } catch (Exception ex3) {
            logger.catching((Throwable)ex3);
            return null;
        }
    }
    ...
    extends ClassRemapper {
        ...
        public InnerClassAdapter(ClassVisitor cv2, InnerClassInfo info) {
            super(327680, cv2, info);
            this.info = info;
        }
