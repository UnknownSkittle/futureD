...
extends AnnotatedMixinElementHandler {
    ...
    public void registerInjector(AnnotatedElementInjector elem) {
        ...
        for (String reference : elem.getAnnotation().getList("method")) {
            ...
            try {
                ...
                elem.printMessage(this.ap, Diagnostic.Kind.ERROR, ex2.getMessage());
            }
    ...
    private boolean registerInjector(AnnotatedElementInjector elem, String reference, MemberInfo targetMember, TypeHandle target) {
        ...
        try {
            ...
            if (elem.hasCoerceArgument() && targetMember.owner == null && targetMember.desc == null) {
                MemberInfo oldMember = MemberInfo.parse(ex2.getOld());
                MemberInfo newMember = MemberInfo.parse(ex2.getNew());
            ...
            elem.printMessage(this.ap, Diagnostic.Kind.ERROR, conflictType + " reference conflict for " + targetName + ": " + reference + " -> " + ex2.getNew() + " previously defined as " + ex2.getOld());
        }
        return true;
    }
    ...
    protected final void remapReference(String subject, String reference, AnnotatedElementInjectionPoint elem) {
        ...
        try {
            ...
            this.ap.printMessage(Diagnostic.Kind.ERROR, ex2.getMessage(), (Element)elem.getElement(), errorsOn);
        }
        try {
        ...
        } catch (ReferenceManager.ReferenceConflictException ex3) {
            this.ap.printMessage(Diagnostic.Kind.ERROR, "Unexpected reference conflict for " + subject + ": " + reference + " -> " + ex3.getNew() + " previously defined as " + ex3.getOld(), (Element)elem.getElement(), errorsOn);
            return;
        }
        elem.notifyRemapped();
    }
