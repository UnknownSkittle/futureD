    ...
    private void registerShadowForTarget(AnnotatedElementShadow<?, ?> elem, TypeHandle target) {
        ...
        for (ObfuscationType type : obfData) {
            try {
                ...
                elem.printMessage(this.ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Shadow " + elem + ": " + ex2.getNew().getSimpleName() + " for target " + target + " conflicts with existing mapping " + ex2.getOld().getSimpleName());
            }
        }
    }
