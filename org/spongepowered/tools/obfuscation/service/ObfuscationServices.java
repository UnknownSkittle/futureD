...
public final class ObfuscationServices {
    ...
    public void initProviders(IMixinAnnotationProcessor ap2) {
        try {
            for (IObfuscationService service : this.serviceLoader) {
                ...
                for (ObfuscationTypeDescriptor obfType : obfTypes) {
                    try {
                        ...
                        ap2.printMessage(Diagnostic.Kind.NOTE, serviceName + " supports type: \"" + type + "\"");
                    } catch (Exception ex2) {
                        ex2.printStackTrace();
                    }
                }
            ...
            ap2.printMessage(Diagnostic.Kind.ERROR, serviceError.getClass().getSimpleName() + ": " + serviceError.getMessage());
