...
extends AnnotatedMixinElementHandler {
    ...
    private boolean registerOverwriteForTarget(AnnotatedElementOverwrite elem, TypeHandle target) {
        ...
        try {
            ...
            elem.printMessage(this.ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Overwrite method: " + ex2.getNew().getSimpleName() + " for target " + target + " conflicts with existing mapping " + ex2.getOld().getSimpleName());
            return false;
        }
        return true;
    }
