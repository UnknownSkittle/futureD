...
extends BasicInterpreter {
    ...
    protected BasicVerifier(int n2) {
        super(n2);
    }
    ...
    public BasicValue naryOperation(AbstractInsnNode abstractInsnNode, List list) throws AnalyzerException {
        ...
        if (n2 == 197) {
            for (int i2 = 0; i2 < list.size(); ++i2) {
                if (BasicValue.INT_VALUE.equals(list.get(i2))) continue;
                throw new AnalyzerException(abstractInsnNode, null, BasicValue.INT_VALUE, (Value)list.get(i2));
            }
        } else {
            Object object;
            int n3 = 0;
            ...
            if (n2 != 184 && n2 != 186) {
                ...
                if (!this.isSubTypeOf((BasicValue)list.get(n3++), this.newValue((Type)object))) {
                    throw new AnalyzerException(abstractInsnNode, "Method owner", this.newValue((Type)object), (Value)list.get(0));
                }
            }
            object = n2 == 186 ? ((InvokeDynamicInsnNode)abstractInsnNode).desc : ((MethodInsnNode)abstractInsnNode).desc;
            Type[] typeArray = Type.getArgumentTypes((String)object);
            while (n3 < list.size()) {
                BasicValue basicValue;
                BasicValue basicValue2 = this.newValue(typeArray[n4++]);
                if (this.isSubTypeOf(basicValue = (BasicValue)list.get(n3++), basicValue2)) continue;
                throw new AnalyzerException(abstractInsnNode, "Argument " + n4, basicValue2, basicValue);
            }
        }
