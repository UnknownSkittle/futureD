...
public class Frame {
    ...
    public Value getLocal(int n2) throws IndexOutOfBoundsException {
        if (n2 >= this.locals) {
    ...
    public void setLocal(int n2, Value value) throws IndexOutOfBoundsException {
        if (n2 >= this.locals) {
    ...
    public Value getStack(int n2) throws IndexOutOfBoundsException {
    ...
    public void execute(AbstractInsnNode abstractInsnNode, Interpreter interpreter) throws AnalyzerException {
        switch (abstractInsnNode.getOpcode()) {
            ...
            case 58: {
                ...
                this.setLocal(n2, value3);
                if (value3.getSize() == 2) {
                    this.setLocal(n2 + 1, interpreter.newValue(null));
                }
                if (n2 <= 0 || (value = this.getLocal(n2 - 1)) == null || value.getSize() != 2) return;
                this.setLocal(n2 - 1, interpreter.newValue(null));
                return;
            }
            case 79: 
    ...
    public boolean merge(Frame frame, Interpreter interpreter) throws AnalyzerException {
        ...
        for (int i2 = 0; i2 < this.locals + this.top; ++i2) {
            Value value = interpreter.merge(this.values[i2], frame.values[i2]);
    ...
    public String toString() {
        ...
        for (n2 = 0; n2 < this.getLocals(); ++n2) {
            stringBuffer.append(this.getLocal(n2));
        }
        stringBuffer.append(' ');
        for (n2 = 0; n2 < this.getStackSize(); ++n2) {
            stringBuffer.append(this.getStack(n2).toString());
        }
