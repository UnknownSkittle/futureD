...
public final class TypeResolver {
            ...
            block6: {
                block4: {
                    ...
                    if (RESOLVES_LAMBDAS && subType.isSynthetic()) {
                        Class fi2;
                        Class clazz = genericType instanceof ParameterizedType && ((ParameterizedType)genericType).getRawType() instanceof Class ? (Class)((ParameterizedType)genericType).getRawType() : (fi2 = genericType instanceof Class ? (Class)genericType : null);
                        if (fi2 != null && fi2.isInterface()) {
    ...
    private static void populateLambdaArgs(Class<?> functionalInterface, Class<?> lambdaType, Map<TypeVariable<?>, Type> map) {
        if (RESOLVES_LAMBDAS) {
            for (Method m2 : functionalInterface.getMethods()) {
                Method objectMethod;
                if (TypeResolver.isDefaultMethod(m2) || Modifier.isStatic(m2.getModifiers()) || m2.isBridge() || (objectMethod = OBJECT_METHODS.get(m2.getName())) != null && Arrays.equals(m2.getTypeParameters(), objectMethod.getTypeParameters())) continue;
                Type returnTypeVar = m2.getGenericReturnType();
                Type[] paramTypeVars = m2.getGenericParameterTypes();
    ...
    private static boolean isDefaultMethod(Method m2) {
        return JAVA_VERSION >= 1.8 && m2.isDefault();
    }
